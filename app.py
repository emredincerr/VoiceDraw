...
with col_audio:
    ...
    st.divider()
    status_message = st.info(body=st.session_state.recording_status)
    st.divider()
    ...
    last_image_use = st.checkbox(label="Son Görseli Kullan")  
...
with col_image:
    ...
    if stop_btn:
        ...
        st.session_state.append({"role": "user", "content": voice_prompt})
